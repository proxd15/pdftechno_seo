...
export default function SignUpPage() {
  ...
  const validateForm = () => {
    ...
    // Add these new checks to match Django's password validators
    ...
    if (commonPasswords.includes(formData.password.toLowerCase())) {
      setFormError('Password is too common');
      return false;
    }
    
    // Check if password is too similar to email or name
    if (formData.password.toLowerCase().includes(formData.email.split('@')[0].toLowerCase()) ||
        formData.password.toLowerCase().includes(formData.name.toLowerCase())) {
      setFormError('Password is too similar to your personal information');
      return false;
    }
