/**
 * Enhanced PDF Rotation API client
 * Handles communication with the backend API for PDF rotation with page-level rotation support
 */
import { getAuthToken } from "@/services/authUtils";


// Use the same API base URL as the compression API
...
/**
 * Rotate PDF files with enhanced page rotation support
 * @param {FormData} formData - FormData containing PDF files and rotation data
 * @param {Function} onProgress - Progress callback (0-100)
 * @returns {Promise} - Promise that resolves with rotation results
 */
export const rotatePDFs = async (formData, onProgress) => {
  try {
        ...
        if (xhr.readyState === 3) {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            try {
              ...
              reject(new Error(errorData.error || 'Rotation failed'));
            } catch (e) {
