...
/**
 * PDF Protection API client
 * Handles communication with the backend API for password-protecting PDF files
 */
 ...
 * @param {FormData} formData - FormData containing PDF file, password, and permission level
 * @param {Function} onProgress - Progress callback (0-100)
 * @returns {Promise} - Promise that resolves with protection results
 */
export const protectPDF = async (formData, onProgress) => {
  try {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (xhr.readyState === 3) {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            try {
              ...
              reject({
                ...errorData,
                message: errorData.error || 'PDF protection operation failed',
                status: xhr.status
              });
            } catch (e) {
              reject({
      ...
      xhr.open('POST', `${API_BASE_URL}/api/pdf/pdf-protect/`, true);
...
/**
 * Create FormData for a PDF protection request
 * @param {File} pdfFile - The PDF file to protect
 * @param {string} password - Password to add to the PDF
 * @param {string} permissionLevel - Permission level (print_only, copy_allowed, edit_allowed, full_control)
 * @returns {FormData} - FormData ready to send to the API
 */
export const createProtectionFormData = (pdfFile, password, permissionLevel = 'print_only') => {
