...
/**
 * PDF Unlock API client
 * Handles communication with the backend API for unlocking password-protected PDF files
 */
 ...
 * @param {FormData} formData - FormData containing PDF file and password
 * @param {Function} onProgress - Progress callback (0-100)
 * @returns {Promise} - Promise that resolves with unlock results
 */
export const unlockPDF = async (formData, onProgress) => {
  try {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (xhr.readyState === 3) {
          ...
          if (xhr.status >= 200 && xhr.status < 300) {
            ...
            try {
              ...
              reject({
                ...errorData,
                message: errorData.error || 'PDF unlock operation failed',
                status: xhr.status
              });
            } catch (e) {
              reject({
      ...
      xhr.open('POST', `${API_BASE_URL}/api/pdf/pdf-unlock/`, true);
...
/**
 * Create FormData for a PDF unlock request
 * @param {File} pdfFile - The password-protected PDF file to unlock
 * @param {string} password - Password to unlock the PDF
 * @returns {FormData} - FormData ready to send to the API
 */
export const createUnlockFormData = (pdfFile, password) => {
