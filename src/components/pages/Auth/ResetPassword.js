...
export default function ResetPassword() {
  ...
  const getPasswordStrengthText = () => {
    if (passwordStrength.score <= 2) return t('resetPassword.passwordStrength.weak');
    if (passwordStrength.score <= 3) return t('resetPassword.passwordStrength.fair');
    if (passwordStrength.score <= 4) return t('resetPassword.passwordStrength.good');
    ...
    const validateToken = async () => {
      ...
      try {
        ...
        if (data.valid) {
          ...
          setError(data.message || t('resetPassword.errors.expiredLink'));
        }
      } catch (error) {
        console.error('Token validation error:', error);
        setError(t('resetPassword.errors.validationFailed'));
      } finally {
        setIsValidating(false);
      }
    };
  ...
  const validatePassword = () => {
    ...
    if (formData.new_password.length < 8) {
      setError(t('resetPassword.validation.passwordTooShort'));
      return false;
    }
    
    if (formData.new_password !== formData.confirm_password) {
      setError(t('resetPassword.validation.passwordsDontMatch'));
      return false;
    }
    ...
    if (commonPasswords.includes(formData.new_password.toLowerCase())) {
      setError(t('resetPassword.validation.passwordTooCommon'));
      return false;
    }
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
        <div className="text-center bg-white p-8 rounded-lg shadow-sm max-w-md w-full">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">
            {t('resetPassword.validating.title')}
          </h2>
          <p className="text-gray-600">
            {t('resetPassword.validating.subtitle')}
          </p>
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
        <div className="w-full max-w-md text-center bg-white p-8 rounded-lg shadow-sm">
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4 mb-6">
            <div className="flex items-center">
              ...
              <p className="text-sm text-green-800 font-medium">
                {t('resetPassword.success.securityMessage')}
              </p>
            </div>
          </div>
          ...
          <div className="space-y-3">
            ...
            >
              {t('resetPassword.success.continueToLogin')}
            </Link>
            
            <p className="text-sm text-gray-500">
              {t('resetPassword.success.signInNote')}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="min-h-screen flex items-center justify-center px-4 bg-gray-50">
        <div className="w-full max-w-md text-center bg-white p-8 rounded-lg shadow-sm">
          ...
          <div className="mt-6 text-center">
            <p className="text-sm text-gray-500">
              {t('resetPassword.invalid.needHelp')}{' '}
              <Link href="/contact" className="text-blue-600 hover:text-blue-800">
                {t('resetPassword.invalid.contactSupport')}
              </Link>
            </p>
          </div>
        </div>
              ...
              <div className="mt-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div className={`flex items-center ${passwordStrength.length ? 'text-green-600' : 'text-gray-400'}`}>
                    ...
                    {t('resetPassword.requirements.length')}
                  </div>
              ...
              <div className="mt-2 flex items-center text-sm">
                {formData.new_password === formData.confirm_password ? (
                  <div className="flex items-center text-green-600">
                    ...
                    {t('resetPassword.feedback.passwordsMatch')}
                  </div>
                ) : (
                  <div className="flex items-center text-red-600">
                    <AlertCircle className="w-4 h-4 mr-1" />
                    {t('resetPassword.feedback.passwordsDontMatch')}
                  </div>
                )}
              </div>
            )}
          ...
          <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
            <div className="flex items-start">
              ...
              <div className="text-xs text-blue-800">
                <p className="font-medium">{t('resetPassword.securityTips.title')}:</p>
                <ul className="mt-1 space-y-0.5">
                  <li>• {t('resetPassword.securityTips.unique')}</li>
                  <li>• {t('resetPassword.securityTips.manager')}</li>
                  <li>• {t('resetPassword.securityTips.share')}</li>
                </ul>
              </div>
            </div>
          </div>
