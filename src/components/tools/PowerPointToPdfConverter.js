...
import { convertPowerPointToPDF, getDownloadUrl, downloadFile, formatFileSize } from '../../api/office_to_pdf_api';
...
const PowerPointToPdfConverter = () => {
  ...
  // Using 400 KB/s as estimation base plus extra processing time for slides
  const estimatedTime = file ? Math.ceil(file.size / (400 * 1024) * 1.5) : 0; // 50% more time for PowerPoint
  ...
  const validateFile = (uploadedFile) => {
    ...
    if (!validExtensions.includes(fileExtension)) {
      setError(`Invalid file format. Only .ppt, .pptx, and .odp files are supported.`);
      return false;
    }
  ...
  const handleConvertToPdf = async () => {
    if (!file) {
      setError("Please select a PowerPoint presentation first.");
      return;
    }
    ...
    try {
      ...
      setError(error.message || 'PowerPoint to PDF conversion failed. Please try again.');
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    if (result) {
      try {
        ...
        setPreviewModal({
          isOpen: true,
          pdfUrl: objectUrl,
          fileName: file ? file.name.replace(/\.[^/.]+$/, "") + ".pdf" : 'Converted Presentation.pdf'
        });
      } catch (error) {
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mb-8">
      ...
      <ModalLoader
        ...
        text={"Converting PowerPoint to PDF..."}
      />
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl mt-4 font-bold mb-2">
          <span className="text-[#DA1F10]">PowerPoint </span> to PDF
        </h1>
        <p className="text-gray-600">Convert your PowerPoint presentations to PDF with ease</p>
        <p className="text-sm text-gray-500 mt-2">Supports .ppt, .pptx, .odp • Max 20 MB</p>
      </div>
        ...
        <div className="w-full md:w-3/4">
          ...
          {!file && !result && (
           <SelectFiles 
              ...
              buttonSize="large"
              multiple={false}
              acceptedFileTypes=".ppt, .pptx, .odp"
          ...
          {file  && (
            <div className="w-full">
              ...
              <div className="mb-6 flex justify-between items-center">
                <h2 className="text-lg font-medium">Selected Presentation</h2>
                <button
  ...
  Remove
              ...
              >
                <p className="text-gray-500 text-sm">
                  Drag and drop a different PowerPoint presentation here or{' '}
                  <button 
                ...
                <div className="flex p-4 items-center border-b border-gray-100">
                  <div className="bg-orange-50 p-3 rounded-lg mr-4">
                    <svg className="w-8 h-8 text-orange-600" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path d="M14 2V8H20L14 2Z" fill="#D24726"/>
                      <path d="M12 14H9V15H12V16H10C9.45 16 9 16.45 9 17V19C9 19.55 9.45 20 10 20H12C12.55 20 13 19.55 13 19V15C13 14.45 12.55 14 12 14ZM12 19H10V17H12V19ZM15 15V14C15 13.45 14.55 13 14 13H11V14H14V15H12C11.45 15 11 15.45 11 16V19C11 19.55 11.45 20 12 20H14C14.55 20 15 19.55 15 19V18H12V17H14C14.55 17 15 16.55 15 16V15Z" fill="white"/>
                    </svg>
                  </div>
          ...
          {result && (
            <div ref={resultSectionRef}>
              <DownloadSection
                files={[{
                  id: result.file_id,
                  name: file ? file.name.replace(/\.[^/.]+$/, "") + ".pdf" : 'Converted Presentation.pdf',
        ...
        {(file || result) && (
          <div className="w-full md:w-1/4 md:sticky md:top-24 h-fit">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm mb-6">
              ...
              {/* Tips Section */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2">About PowerPoint to PDF</h4>
                <ul className="text-sm text-gray-600 space-y-3">
                  <li className="flex">
                    ...
                    <span>Preserves slide layouts, graphics and animations</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Creates one PDF page per slide</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Supports presentations up to 20 MB in size</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Perfect for sharing presentations securely</span>
                  </li>
                </ul>
              </div>
            </div>
