...
import { convertPDFToPowerPoint, getPowerPointDownloadUrl, downloadFile, formatFileSize } from '../../api/pdf_to_office_api';
...
const PdfToPowerPointConverter = () => {
  ...
  const estimatedTime = file ? Math.ceil(file.size / (250 * 1024) * 2.2) : 0; // Slower for PowerPoint
  ...
  // Handle conversion to PowerPoint
  const handleConvertToPowerPoint = async () => {
    ...
    try {
      const result = await convertPDFToPowerPoint(
        file,
        {
          outputFilename: file.name.replace(/\.[^/.]+$/, "") + ".pptx"
        },
      ...
      setError({
        type: 'conversion',
        message: error.message || 'PDF to PowerPoint conversion failed. Please try again.',
        action: null
      });
    } finally {
      setIsProcessing(false);
  ...
  const handleDownload = (fileId, fileName) => {
    const finalFileName = fileName.toLowerCase().endsWith('.pptx') ? fileName : `${fileName}.pptx`;
    const downloadUrl = getPowerPointDownloadUrl(fileId, finalFileName);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mb-8">
      ...
      <ModalLoader
        ...
        text={"Converting PDF to PowerPoint..."}
      />
      
      {/* Title */}
      <div className="text-center mb-10">
        <h1 className="text-4xl mt-4 font-bold mb-2">
          <span className="text-[#DA1F10]">PDF </span> to PowerPoint
        </h1>
        ...
        <div className="w-full md:w-3/4">
          ...
          {file && (
            <div className="w-full">
              ...
              {/* Drop Zone */}
              <div
                className={`w-full border-2 border-dashed rounded-lg mb-6 p-4 text-center drop-zone ${isDragging ? 'border-red-400 bg-red-50' : 'border-gray-200 bg-gray-50'} transition-all duration-200 hover:border-red-300`}
          ...
          {result && (
            <div ref={resultSectionRef}>
              <DownloadSection
                files={[{
                  id: result.file_id,
                  name: file ? file.name.replace(/\.[^/.]+$/, "") + ".pptx" : 'Converted Presentation.pptx',
        ...
        {(file || result) && (
          <div className="w-full md:w-1/4 md:sticky md:top-24 h-fit">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm mb-6">
              ...
              {/* Tips Section */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2">About PDF to PowerPoint</h4>
                <ul className="text-sm text-gray-600 space-y-3">
                  <li className="flex">
                    ...
                    <span>Converts each page to slides</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Preserves images and layouts</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Creates editable presentations</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
