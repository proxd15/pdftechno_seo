...
import { convertPDFToWord, getWordDownloadUrl, downloadFile, formatFileSize } from '../../api/pdf_to_office_api';
...
const PdfToWordConverter = () => {
  ...
  const estimatedTime = file ? Math.ceil(file.size / (300 * 1024) * 1.8) : 0;
  ...
  const handleConvertToWord = async () => {
    ...
    try {
      ...
      setError({
        type: 'conversion',
        message: error.message || 'PDF to Word conversion failed. Please try again.',
        action: null
      });
    } finally {
      setIsProcessing(false);
  ...
  // Handle downloading the converted Word document
  const handleDownload = (fileId, fileName) => {
    const finalFileName = fileName.toLowerCase().endsWith('.docx') ? fileName : `${fileName}.docx`;
    const downloadUrl = getWordDownloadUrl(fileId, finalFileName);
  ...
  return (
    <div className="w-full max-w-7xl mx-auto mb-8">
      ...
      <ModalLoader
        ...
        text={"Converting PDF to Word..."}
      />
      
      {/* Title */}
      <div className="text-center mb-10">
        <h1 className="text-4xl mt-4 font-bold mb-2">
          <span className="text-[#DA1F10]">PDF </span> to Word
        </h1>
          ...
          {result && (
            <div ref={resultSectionRef}>
              <DownloadSection
                files={[{
                  id: result.file_id,
                  name: file ? file.name.replace(/\.[^/.]+$/, "") + ".docx" : 'Converted Document.docx',
            ...
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm mb-6">
              ...
              <div className="mb-6 border-b border-gray-200 pb-4">
                <ul className="text-sm text-gray-600 space-y-2">
                  <li className="flex justify-between">
                    <span>File:</span>
                    <span className="font-medium">{file?.name.slice(0,20)}...</span>
                  </li>
                  <li className="flex justify-between">
                    <span>Size:</span>
              ...
              {file && (
                <button
                  ...
                  className="w-full cursor-pointer py-3 px-4 bg-[#DA1F10] hover:bg-red-700 text-white font-medium rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#2B5797] mb-4"
                >
                  Convert to Word
                </button>
              )}
              ...
              {/* Tips Section */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2">About PDF to Word</h4>
                <ul className="text-sm text-gray-600 space-y-3">
                  <li className="flex">
                    ...
                    <span>Extracts text and maintains formatting</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Preserves images and tables</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Creates editable Word documents</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Perfect for document editing</span>
                  </li>
                </ul>
              </div>
            </div>
