// components/tools/PDFtoPDFA.jsx

"use client"

import { useState, useRef, useEffect } from 'react';
import { convertPDFtoPDFA, createConversionFormData, getDownloadUrl, downloadFile } from '../../api/pdftopdfa_api';
...
const PDFtoPDFA = () => {
  ...
  const handleDecryptFile = (forConversion = false) => {
    setPasswordModal({
      isOpen: true,
      error: '',
      password: '',
  ...
  const { password, isForConversion } = passwordModal;
  if (!password.trim()) {
  ...
  try {
    ...
    // Close the modal
    setPasswordModal({ isOpen: false, error: '', password: '', isForConversion: false });
  ...
  // Add this function to the PDFtoPDFA component
const handleFileChange = (e) => {
...
}, [file, decryptedPassword]); // Add decryptedPassword as a dependency
  ...
  const handleConvertPDF = async (password = null) => {
    ...
    try {
      ...
      const formData = createConversionFormData(file, pdfPassword);
      // Call the API to convert PDF
      const result = await convertPDFtoPDFA(
        formData,
      ...
      if (error.requires_password) {
        ...
        setError(error.message || 'PDF to PDF/A conversion failed. Please try again.');
      }
    } finally {
      setIsProcessing(false);
    }
    ...
    <div className="w-full max-w-7xl mx-auto mb-8">
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl mt-4 font-bold mb-2">
          <span className="text-[#DB1E10]">PDF to</span> PDF/A
        </h1>
        <p className="text-gray-600">Convert your PDFs to PDF/A format for long-term archiving</p>
            ...
            <div className="w-full mb-6">
              ...
              <div className="border rounded-xl overflow-hidden bg-white shadow-sm">
                <div className="px-4 py-3 bg-gray-50 border-b">
                  <h3 className="font-medium">PDF to Convert</h3>
                </div>
                <div className="p-4">
                  <div className="flex flex-col md:flex-row items-center">
                    ...
                    <div className="w-full md:w-2/3 p-4">
                      ...
                      <div className="mt-4">
                        <h5 className="font-medium mb-1">About PDF/A Format</h5>
                        <p className="text-sm text-gray-600">
                          PDF/A is designed for long-term archiving of electronic documents. Converting to PDF/A ensures your document remains accessible in the future, regardless of software or hardware changes.
                        </p>
                      </div>
                    </div>
                  </div>
          ...
          <div className="w-full md:w-1/4 md:sticky md:top-24 h-fit">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm">
              ...
              >
                ...
                {!(preview === 'invalid' || (preview === 'encrypted' && !isEncrypted)) && estimatedUploadTime > 0 && (
                  <span className="text-xs block mt-1">
              ...
              <div className="mt-6 text-sm text-gray-600">
                <h4 className="font-medium text-gray-700 mb-2">PDF/A Benefits</h4>
                <ul className="space-y-2">
                  <li className="flex items-start">
                    ...
                    <span>Long-term document preservation</span>
                  </li>
                  <li className="flex items-start">
                    <svg className="w-5 h-5 text-[#DB1E10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                    <span>Self-contained (all fonts embedded)</span>
                  </li>
                  <li className="flex items-start">
                    <svg className="w-5 h-5 text-[#DB1E10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                    <span>Compliance with archival standards</span>
                  </li>
                  <li className="flex items-start">
                    <svg className="w-5 h-5 text-[#DB1E10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 13l4 4L19 7"></path>
                    </svg>
                    <span>PDF/A-3 standard (latest version)</span>
                  </li>
                </ul>
              </div>
