...
import { convertWordToPDF, getDownloadUrl, downloadFile, formatFileSize } from '../../api/office_to_pdf_api';
...
const WordToPdfConverter = () => {
  ...
  const estimatedTime = file ? Math.ceil(file.size / (400 * 1024)) : 0;
  ...
  const validateFile = (uploadedFile) => {
    ...
    if (!validExtensions.includes(fileExtension)) {
      setError(`Invalid file format. Only .doc, .docx, .rtf, and .odt files are supported.`);
      return false;
    }
  ...
  const handleConvertToPdf = async () => {
    ...
    try {
      ...
      setError(error.message || 'Word to PDF conversion failed. Please try again.');
    } finally {
      setIsProcessing(false);
    }
  };
    ...
    if (result) {
      try {
        ...
        setPreviewModal({
          isOpen: true,
          pdfUrl: objectUrl,
          fileName: file ? file.name.replace(/\.[^/.]+$/, "") + ".pdf" : 'Converted Document.pdf'
        });
      } catch (error) {
    ...
    <div className="w-full max-w-7xl mx-auto mb-8">
      ...
      <div className="text-center mb-10">
        <h1 className="text-4xl mt-4 font-bold mb-2">
          <span className="text-[#DA1F10]">Word </span> to PDF
        </h1>
        <p className="text-gray-600">Convert your Word documents to PDF with ease</p>
        <p className="text-sm text-gray-500 mt-2">Supports .doc, .docx, .rtf, .odt • Max 20 MB</p>
      </div>
        ...
        <div className="w-full md:w-3/4">
          ...
          {!file && !result && (
            <SelectFiles 
              ...
              onError={setError}
              buttonText="Select Word Document"
              buttonColor="red"
              buttonSize="large"
              multiple={false}
  ...
  Remove
              ...
              >
                <p className="text-gray-500 text-sm">
                  Drag and drop a different Word document here or{' '}
                  <button 
                ...
                <div className="flex p-4 items-center border-b border-gray-100">
                  <div className="bg-blue-50 p-3 rounded-lg mr-4">
                    <svg className="w-8 h-8 text-blue-700" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                      ...
                      <path d="M14 2V8H20L14 2Z" fill="#2B579A"/>
                      <path d="M9.5 14.5L8 18H10L10.5 16.5H13.5L14 18H16L14.5 14.5H13L12 17L11 14.5H9.5Z" fill="white"/>
                    </svg>
                  </div>
          ...
          {result && (
            <div ref={resultSectionRef}>
              <DownloadSection
                files={[{
                  id: result.file_id,
                  name: file ? file.name.replace(/\.[^/.]+$/, "") + ".pdf" : 'Converted Document.pdf',
        ...
        {file && (
          <div className="w-full md:w-1/4 md:sticky md:top-24 h-fit">
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200 shadow-sm mb-6">
              ...
              {/* Tips Section */}
              <div>
                <h4 className="text-sm font-medium text-gray-700 mb-2">About Word to PDF</h4>
                <ul className="text-sm text-gray-600 space-y-3">
                  <li className="flex">
                    ...
                    <span>Preserves text formatting, images, and tables</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Creates PDF that can be viewed on any device</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                    </svg>
                    <span>Supports documents up to 20 MB in size</span>
                  </li>
                  <li className="flex">
                    <svg className="w-5 h-5 text-[#DA1F10] mr-2 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
